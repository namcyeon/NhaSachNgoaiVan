@if(count($designer_img_elments) > 0 && count($designer_hf_data) >0)
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="overflow:auto;">
      <div class="canvas-container-main">
        ...
        <div id="swap-popover-content" class="hide">
          ...
          <div data-id="{{ $row->id }}" data-design_img_url="{{ $row->design_img_url }}" data-design_trans_img_url="{{ $row->design_trans_img_url }}" class="design-title-items"><div><img src="{{ $row->design_title_icon }}"></div><div class="design-title-label">{!! $row->title_label !!}</div></div>
          @endforeach
        </div>
...
<input type="hidden" name="track_is_loaded_first" id="track_is_loaded_first" value="{{ $designer_img_elments[0]->design_img_url }}">
<input type="hidden" name="track_is_trans_loaded_first" id="track_is_trans_loaded_first" value="{{ $designer_img_elments[0]->design_trans_img_url }}">
<input type="hidden" name="hf_design_save_json_data" id="hf_design_save_json_data" value="{{ htmlspecialchars($design_save_data) }}">
