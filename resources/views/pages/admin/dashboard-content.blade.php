...
<div class="row">
  ...
  <div class="col-lg-3 col-xs-12">
    <div class="small-box bg-aqua">
      ...
      <a href="{{ route('admin.product_list')}}" class="small-box-footer">{{ trans('admin.more_info') }} <i class="fa fa-arrow-circle-right"></i></a>
    </div>
  </div>
  <div class="col-lg-3 col-xs-12">
  ...
  <section class="col-lg-7">
    <div class="box box-info">
      <div class="box-header with-border">
        <h3 class="box-title">{{ trans('admin.latest_orders') }}</h3>
    ...
    <div class="box box-info">
      <div class="box-header with-border">
        <h3 class="box-title">{{ trans('admin.recently_added_products') }}</h3>
        <div class="box-tools pull-right">
          <button class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
        </div>
      </div>
            ...
            @foreach($dashboard_data['latest_products'] as $vals)
              <li class="item">
                <div class="product-img">
                  @if($vals['img_url'])
                    <img src="{{ $vals['img_url'] }}" alt="">
                  @else
                    <img src="{{ asset('public/images/no-image.png') }}" alt="">
                  @endif
                </div>
                <div class="products-info">
                  <a target="_blank" href="{{ route('details-page', string_slug_format(get_product_title($vals['id']))) }}" class="product-title">{!! $vals['title'] !!} <span class="label label-warning pull-right">{!! $vals['price'] !!}</span></a>
                  <span class="product-description">
                    {!! string_decode($vals['description']) !!}
                  </span>
                </div>
              </li>
            @endforeach
      ...
      <div class="box-footer clearfix">
        <a href="{{ route('admin.product_list') }}" class="btn btn-sm btn-default btn-flat pull-right">{{ trans('admin.view_all_products') }}</a>
      </div>
    </div>
  </section>
