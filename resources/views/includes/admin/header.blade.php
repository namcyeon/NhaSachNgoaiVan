<header class="main-header">
  <div class="logo">
    <a target="_blank" title="{{ trans('admin.view_online_store') }}" href="{{ route('root') }}"><i class="fa fa-shopping-cart" aria-hidden="true"></i> {!! trans('admin.view_online_store') !!}</a>
  </div>
        ...
        <li class="dropdown user user-menu">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown">
            @if(!empty($user_data['user_photo_url']))
              <img src="{{ asset('public/uploads/'. $user_data['user_photo_url']) }}" class="user-image" alt=""/>
            @else
              <img src="{{ asset('public/images/user-outline.png') }}" class="user-image" alt=""/>
            @endif
