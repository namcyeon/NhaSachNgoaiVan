...
$(document).ready(function(){
      ...
      init: function() 
      {
        ...
        this.on("success", function(file, responseText) 
        {
          if(responseText.status === 'success')
          {
            ...
            $('#hf_frontend_profile_picture').val( responseText.name );
  ...
  $(window).resize(function(){
	...
	if($('#table_user_account_order_list').length>0 || $('#coupons_list').length>0){
		$('#table_user_account_order_list, #coupons_list').DataTable();
	}
    ...
    if($('.payment-options').length>0){
      $('.payment-options input[type="radio"]').on('ifClicked', function(event){
        if(this.value === 'paypal'){
          ...
          $('#bacsPopover, #codPopover, #stripePopover').hide();
          $('#paypalPopover').show();
        }
        else if(this.value === 'bacs'){
          $('.place-order').text( frontendLocalizationString.place_order );
          $('#paypalPopover, #codPopover, #stripePopover').hide();
          $('#bacsPopover').show();
        }
        else if(this.value === 'cod'){
          $('.place-order').text( frontendLocalizationString.place_order );
          $('#paypalPopover, #bacsPopover, #stripePopover').hide();
          $('#codPopover').show();
        }
        else if(this.value === 'stripe'){
          $('.place-order').text( frontendLocalizationString.proceed_to_stripe );
          $('#paypalPopover, #bacsPopover, #codPopover').hide();
          $('#stripePopover').show();
        }
          ...
          success: function(data){
            if(data.status == 'success' && data.notice_type == 'user_wishlist_saved'){
              swal({
                ...
                imageUrl: $('#hf_base_url').val() + '/resources/assets/images/thumbs-up.jpg'
              },
              function(){
