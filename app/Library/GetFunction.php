...
class GetFunction
{
  ...
  {
    $getOrdersByDate    =   Product::whereBetween('created_at', array($start_date, $end_date.' 23:59:59'))->where('post_type', 'shop_order')->orderBy('created_at', 'DESC')->get();
  ...
  {
    $getOrdersByYear    =   Product::whereBetween('created_at', [ $this->carbonObject->today()->startOfYear(), $this->carbonObject->today()->endOfYear() ])->where('post_type', 'shop_order')->orderBy('created_at', 'DESC')->get();
  ...
  public function get_order_by_specific_date_range($daysOrders)
  {
    ...
    foreach($daysOrders as $rows)
    {
      ...
      $order_data['order_status']   = ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_order_status'])->first()->key_value;
      $order_data['order_totals']   = ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_order_total'])->first()->key_value;
      $order_data['order_totals_with_currency']   = price_html( ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_order_total'])->first()->key_value );
  ...
  public function get_reports_payment_method_data($data)
  {
    ...
    {
      $index = ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_payment_method'])->first()->key_value;
      $order_total = ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_order_total'])->first()->key_value;
  ...
  public function sendCustomMail($data)
  {
    ...
    if($data['source'] == 'order_confirmation')
    {
      ...
      $get_post_meta    =   ProductExtra:: where('post_id', $data['data']['order_id'])->get();
  ...
  public static function product_img($product_id){
    $image_data = '';
    $get_post_meta    =   ProductExtra:: where(['post_id' => $product_id, 'key_name' => '_product_related_images_url'])->first();
  ...
  public static function product_price($product_id){ 
    $final_price = 0;
    $_this            =   new self;
    $get_post_meta    =   ProductExtra:: where(['post_id' => $product_id, 'key_name' => '_product_price'])->first();
  ...
  public static function products_by_product_tag_slug( $slug ){
    ...
    if(isset($slug)){
      $get_tag_by_slug = ProductTerm::where(['slug' => $slug, 'type' => 'product_tag'])->get()->toArray();
  ...
  public static function product_type( $product_id = '' ){
    $product_type =  '';
    $post_meta    =  ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_type'])->first();
  ...
  public static function check_design_enable_by_product_id($product_id){
    if($product_id > 0){
      $post_meta   =   ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_as_custom_design'])->first();
  ...
  public static function product_review($product_id)
  {
    if($product_id)
    {
      ...
      $post_meta_1   =   ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews'])->first();
      $post_meta_2   =   ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_add_link_to_product_page'])->first();
      $post_meta_3   =   ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_add_link_to_details_page'])->first();
      $post_meta_4   =   ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_totals_with_link'])->first();
  ...
  public static function product_availability( $product_id ){
    ...
    $post_meta  =  ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_manage_stock_availability'])->first();
  ...
  public static function customer_order_billing_shipping_info($order_id){
    ...
    if(isset($order_id) &&  $order_id > 0){
      $get_order_user = ProductExtra::where(['post_id' => $order_id, 'key_name' => '_customer_user'])->first();
      
      if(!empty($get_order_user)){
        $order_user = unserialize($get_order_user->key_value);
        
        if($order_user['user_mode'] == 'guest'){
          $get_order_post_meta    =   ProductExtra:: where('post_id', $order_id)->get();
