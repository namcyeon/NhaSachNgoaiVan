...
class FrontendManagerController extends Controller
{
  ...
  public function goToDifferentPages($params = null, $params2 = null)
  {
    ...
    if(($params && (Request::is('product/details/*'))) || ($params && (Request::is('san-pham/dang-xem/*'))) || Request::is('product/customize/*')){
      $get_post = Product::where(['post_slug' => $params, 'post_status' => 1])->get()->toArray();
    ...
    if(($params && Request::is('blog/*')) || ($params && Request::is('xem-tin/*'))){
      ...
      if(count($parse_url) > 0 && isset($parse_url[1])){
        ...
        if(File::exists($file_path)){
          ...
          if(!empty($get_extension) && !empty($get_content_type)){
            ...
            $user_id = 0;
            $get_post = ProductExtra::where(['post_id' => $order_id, 'key_name' => '_order_process_key'])->first();
