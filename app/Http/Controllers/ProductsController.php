...
use shopist\Models\ProductTerm;
use shopist\Models\ProductTermExtra;
use shopist\Models\ProductRelationship;
...
class ProductsController extends Controller
{
   ...
   */
    ...
    if($search_val && $search_val != ''){
      $get_term_data = ProductTerm:: where($where)
    ...
    if(count($get_term_data) >0){
      foreach($get_term_data as $term){
        $term_extra = ProductTermExtra:: where(['term_id' => $term['term_id']])
                      ->get();
   ...
   */
    ...
    $get_term_data = ProductTerm:: where(['term_id' => $term_id])
                     ->get()
                     ->toArray();
  ...
  public function get_categories_name($cat_type){
    ...
    $get_cats         =   ProductTerm:: where(['type' => $cat_type, 'status' => 1])
                          ->get()
                          ->toArray();
  ...
  public function get_categories($cat_id = 0, $cat_type){
    $get_categories_data  =  ProductTerm::where(['type' => $cat_type, 'parent' => $cat_id, 'status' => 1])->get();
    ...
    if($get_categories_data->count() > 0){
      foreach ($get_categories_data as $mainCategory){
        ...
        $category['parent']						     =  $mainCategory->parent;
				...
				$term_extra = ProductTermExtra:: where(['term_id' => $mainCategory->term_id])
                      ->get();
  ...
  public function get_parent_categories($cat_id = 0, $cat_type){
    ...
    $get_categories_data  =  ProductTerm::where(['type' => $cat_type, 'parent' => $cat_id, 'status' => 1])->get()->toArray();
    ...
    if( Request::isMethod('post') && Session::token() == Input::get('_token') ){
      ...
      if($validator->fails()){
        ...
        ->withErrors( $validator );
            ...
            if(Input::has('downloadable_file_name') && Input::has('downloadable_uploaded_file_url') && Input::has('downloadable_online_file_url') && count($file_name) > 0 && count($uploaded_file_url) > 0 && count($online_file_url)){
              foreach($file_name as $key => $name){
                $downloadable_product_data[$key] = array('file_name' => $name, 'uploaded_file_url' => $uploaded_file_url[$key], 'online_file_url' => $online_file_url[$key]);
              }
            }
            ...
            if(!empty($params)){
              $get_post =  Product:: where('post_slug', $params)->get()->toArray();
        ...
        if(Input::has('ProductSKU')){
          $getSKU  =  ProductExtra:: where(['key_name' => '_product_sku', 'key_value' => Input::get('ProductSKU')])->first();
        ...
        if( ( $skuCheck && $sku_id == $product_id ) ||  $skuCheck == null){
          ...
          $post        =  new Product;
          $post_slug   =  '';
          $check_slug  =  Product::where(['post_slug' => string_slug_format( $url_slug )])->orWhere('post_slug', 'like', '%' . string_slug_format( $url_slug ) . '%')->get()->count();
            ...
            if($post->save()){  
              ...
              if(Input::has('inputCategoriesName') && count(Input::get('inputCategoriesName'))>0){
                ...
                if(count($cat_array) > 0){
                  ProductRelationship::insert( $cat_array );    
                }
              }
              ...
              if(Input::has('inputManufacturerName') && count(Input::get('inputManufacturerName'))>0){
                ...
                if(count($manufacturer_array) > 0){
                  ProductRelationship::insert( $manufacturer_array );    
                }
              }
              ...
              if(Input::has('inputTagsName') && count(Input::get('inputTagsName'))>0){
                ...
                if(count($tags_array) > 0){
                  ProductRelationship::insert( $tags_array );    
                }
              }
              ...
              if(Input::has('inputColorsName') && count(Input::get('inputColorsName'))>0){
                ...
                if(count($colors_array) > 0){
                  ProductRelationship::insert( $colors_array );    
                }
              }
              ...
              if(Input::has('inputSizesName') && count(Input::get('inputSizesName'))>0){
                ...
                if(count($sizes_array) > 0){
                  ProductRelationship::insert( $sizes_array );    
                }
              }
          ...
          elseif (Input::get('hf_post_type') == 'update_post'){
            ...
            if( Product::where('id', $product_id)->update($data)){
              ...
              $get_post_seo_title   =   ProductExtra:: where(['post_id' => $product_id, 'key_name' => '_product_seo_title'])->get()->first();
              $get_post_seo_desc    =   ProductExtra:: where(['post_id' => $product_id, 'key_name' => '_product_seo_description'])->get()->first();
              $get_post_seo_key     =   ProductExtra:: where(['post_id' => $product_id, 'key_name' => '_product_seo_keywords'])->get()->first();
              $get_post_compare_data    =   ProductExtra:: where(['post_id' => $product_id, 'key_name' => '_product_compare_data'])->get()->first();
              ...
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_related_images_url'])->update($data_related_url);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_type'])->update($data_product_type);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_sku'])->update($data_product_sku);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_regular_price'])->update($data_regular_price);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_sale_price'])->update($data_sale_price);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_price'])->update($data_product_price);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_sale_price_start_date'])->update($data_sale_price_start_date);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_sale_price_end_date'])->update($data_sale_price_end_date);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_manage_stock'])->update($data_manage_stock);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_manage_stock_qty'])->update($data_manage_stock_qty);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_manage_stock_back_to_order'])->update($data_manage_stock_back_to_order);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_manage_stock_availability'])->update($data_stock_availability);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_extra_features'])->update($data_extra_features);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_as_recommended'])->update($data_enable_recommended);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_as_features'])->update($data_enable_features);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_as_latest'])->update($data_enable_latest);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_as_related'])->update($data_enable_related);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_as_custom_design'])->update($data_enable_custom_design);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_as_selected_cat'])->update($data_enable_home_product);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_taxes'])->update($data_enable_taxes);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_custom_designer_settings'])->update($data_custom_designer_settings);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_custom_designer_data'])->update($data_custom_designer_data);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews'])->update($data_enable_review);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_add_link_to_product_page'])->update($data_p_page);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_reviews_add_link_to_details_page'])->update($data_d_page);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_video_feature'])->update($data_enable_product_video);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_video_feature_display_mode'])->update($data_display_mode);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_video_feature_title'])->update($data_title_for_video);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_video_feature_panel_size'])->update($data_video_feature_panel_size);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_video_feature_source'])->update($data_video_source_name);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_video_feature_source_embedded_code'])->update($data_video_embed_code);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_video_feature_source_online_url'])->update($data_video_online_url);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_manufacturer'])->update($data_enable_manufacturer);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_enable_visibility_schedule'])->update($data_visibilityschedule);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_seo_title'])->update($data_seo_title);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_seo_description'])->update($data_seo_description);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_seo_keywords'])->update($data_seo_keywords);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_product_compare_data'])->update($data_compare_product);
							ProductExtra::where(['post_id' => $product_id, 'key_name' => '_is_role_based_pricing_enable'])->update($data_is_role_based_enable);
							ProductExtra::where(['post_id' => $product_id, 'key_name' => '_role_based_pricing'])->update($data_role_based_pricing);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_downloadable_product_files'])->update($data_downloadable_product_files);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_downloadable_product_download_limit'])->update($data_downloadable_product_download_limit);
              ProductExtra::where(['post_id' => $product_id, 'key_name' => '_downloadable_product_download_expiry'])->update($data_downloadable_product_download_expiry);
              
              
              $is_object_exist = ProductRelationship::where('object_id', $product_id)->get();
              
              if(count($is_object_exist)>0){
                ProductRelationship::where('object_id', $product_id)->delete();
              }
  ...
  public function getProductDataById( $product_id ){
    ...
    $get_post_meta    =   ProductExtra:: where('post_id', $product_id)->get();
  ...
  */
  public function saveManufacturersData($params = null){
    if( Request::isMethod('post') && Session::token() == Input::get('_token') ){
      ...
      if($validator->fails()){
        ...
        $check_slug = ProductTerm::where(['slug' => string_slug_format( Input::get('inputManufacturersName') )])->orWhere('slug', 'like', '%' . string_slug_format( Input::get('inputManufacturersName') ) . '%')->get()->count();
        ...
        if($params && Request::is('admin/manufacturers/update/*')){
          ...
          if( ProductTerm::where('slug', $params)->update($data)){
            ...
            ProductTermExtra::where(['term_id' => $getTerm->term_id, 'key_name' => '_brand_country_name'])->update($brand_country_name);
            ProductTermExtra::where(['term_id' => $getTerm->term_id, 'key_name' => '_brand_short_description'])->update($brand_short_description);
            ProductTermExtra::where(['term_id' => $getTerm->term_id, 'key_name' => '_brand_logo_img_url'])->update($brand_logo_img_url);
          ...
          if( $termObj->save() ){
            if(ProductTermExtra::insert(array(
                                    array(
                                        'term_id'       =>  $termObj->id,
  ...
  public function getBrandDataBySlug($term_slug){
    ...
    if(!empty($get_brand_term)){
      ...
      $get_object_data =  DB::table('product')
                          ->where(['product.post_status' => 1, 'product.post_type' => 'product', 'product_relationships.term_id' => $get_brand_term->term_id ])
                          ->join('product_relationships', 'product_relationships.object_id', '=', 'product.id')
                          ->select('product.*')
  ...
  public function getCatByObjectId($object_id){
    ...
    $get_cat_list  =  DB::table('product_terms')
                      ->where(['product_relationships.object_id' => $object_id, 'product_terms.type' => 'product_cat' ])
                      ->join('product_relationships', 'product_relationships.term_id', '=', 'product_terms.term_id')
  ...
  public function getTagsByObjectId($object_id){
    ...
    $get_tag_list  =  DB::table('product_terms')
                      ->where(['product_relationships.object_id' => $object_id, 'product_terms.type' => 'product_tag' ])
                      ->join('product_relationships', 'product_relationships.term_id', '=', 'product_terms.term_id')
                      ->select('product_terms.*')        
                      ->get()
                      ->toArray();
  ...
  public function getColorsByObjectId($object_id){
    ...
    $get_colors_list  =  DB::table('product_terms')
                         ->where(['product_relationships.object_id' => $object_id, 'product_terms.type' => 'product_colors' ])
                         ->join('product_relationships', 'product_relationships.term_id', '=', 'product_terms.term_id')
                         ->select('product_terms.*')        
                         ->get()
                         ->toArray();
  ...
  public function getSizesByObjectId($object_id){
    ...
    $get_sizes_list  =  DB::table('product_terms')
                        ->where(['product_relationships.object_id' => $object_id, 'product_terms.type' => 'product_sizes' ])
                        ->join('product_relationships', 'product_relationships.term_id', '=', 'product_terms.term_id')
                        ->select('product_terms.*')        
                        ->get()
                        ->toArray();
  ...
  public function getManufacturerByObjectId($object_id){
    ...
    $get_brands_list  = DB::table('product_terms')
                        ->where(['product_relationships.object_id' => $object_id, 'product_terms.type' => 'product_brands' ])
                        ->join('product_relationships', 'product_relationships.term_id', '=', 'product_terms.term_id')
                        ->select('product_terms.*')        
                        ->get()
                        ->toArray();
  ...
  public function getPosts($pagination = false, $search_val = null, $status_flag = -1){
    ...
    if(count($get_posts_for_product)>0){
      foreach($get_posts_for_product as $rows){
        ...
        $get_post_meta    =   ProductExtra:: where('post_id', $rows['id'])->get();
  ...
  public function getProductCommentsList(){
    ...
    $get_comments = DB::table('comments')->where(['comments.target' => 'product'])
                    ->join('product', 'comments.object_id', '=', 'product.id')
                    ->join('users', 'comments.user_id', '=', 'users.id')
                    ->select('comments.*', 'product.post_title', 'product.post_slug', 'users.display_name', 'users.user_photo_url')        
                    ->get()
                    ->toArray();
  ...
  public function getAdvancedProducts(){
    ...
    $get_recommended_items =  ProductExtra::where(['key_name' => '_product_enable_as_recommended', 'key_value' => 'yes'])->get();
    $get_features_items    =  ProductExtra::where(['key_name' => '_product_enable_as_features', 'key_value' => 'yes'])->get();
    $get_latest_items      =  ProductExtra::where(['key_name' => '_product_enable_as_latest', 'key_value' => 'yes'])->get();
    $get_todays_items      =  DB::table('product')
                              ->where('product.post_type', 'shop_order')
                              ->whereDate('product.created_at', '=', $this->carbonObject->today()->toDateString())
                              ->join('orders_items', 'orders_items.order_id', '=', 'product.id')
                              ->orderBy('product.id', 'desc')
                              ->select('orders_items.*')
                              ->limit(10)
                              ->get()
  ...
  public function getFilterProductsDataWithPagination( $filter = array() ){
    ...
    if(isset($filter['selected_colors'])){
      ...
      if(count($parse_colors) > 0){
        ...
        foreach($parse_colors as $color){
          $get_color_term = ProductTerm::where(['slug' => $color, 'type' => 'product_colors'])->first();
    ...
    if(isset($filter['selected_sizes'])){
      ...
      if(count($parse_sizes) > 0){
        ...
        foreach($parse_sizes as $size){
          $get_size_term = ProductTerm::where(['slug' => $size, 'type' => 'product_sizes'])->first();
    ...
    if(count($filter_arr) > 0){
      foreach($filter_arr as $term_filter){
        $get_posts_for_product  =  DB::table('product')
                               ->where(['product.post_status' => 1, 'product.post_type' => 'product']);
        
        if( isset($filter['price_min']) && isset($filter['price_max']) && $filter['price_min'] >= 0 && $filter['price_max'] >=0){
          $get_posts_for_product->where(['post_extras.key_name' => '_product_price', 'product_relationships.term_id' => $term_filter['id']]);
          ...
          $get_posts_for_product->join('post_extras', 'post_extras.post_id', '=', 'product.id');
          $get_posts_for_product->join('product_relationships', 'product_relationships.object_id', '=', 'product.id');
        }

        $get_posts_for_product->select('product.*'); 
  ...
  public function getProductByCatSlug($cat_slug, $filter = array()){
    ...
    $get_term = ProductTerm::where(['slug' => $cat_slug, 'type' => 'product_cat'])->first();
      ...
      if(count($get_child_cat) > 0){
        ...
        foreach($cats_arr as $cat){
          if( (count($filter_arr) > 0 && ($cat['search_type'] == 'color-filter' || $cat['search_type'] == 'size-filter')) || (count($filter_arr) == 0 && $cat['search_type'] == 'category-filter')){
            $get_post_data =  DB::table('product');

            if( isset($filter['price_min']) && isset($filter['price_max']) && $filter['price_min'] >= 0 && $filter['price_max'] >=0){
              $get_post_data->where(['product.post_status' => 1, 'product.post_type' => 'product', 'product_relationships.term_id' => $cat['id'], 'post_extras.key_name' => '_product_price' ]);
            }
            else{
              $get_post_data->where(['product.post_status' => 1, 'product.post_type' => 'product', 'product_relationships.term_id' => $cat['id'] ]);
            }
            ...
            $get_post_data->join('product_relationships', 'product_relationships.object_id', '=', 'product.id');

            if( isset($filter['price_min']) && isset($filter['price_max']) && $filter['price_min'] >= 0 && $filter['price_max'] >=0){
              $get_post_data->join('post_extras', 'post_extras.post_id', '=', 'product.id');
            }

            $get_post_data->select('product.*');
  ...
  public function getAllAttributes($product_id){
    ...
    $get_attr_by_products  =  ProductExtra::where(['post_id' => $product_id, 'key_name' => '_attribute_post_data'])->get()->toArray();
  ...
  public function getProductsByTermId($term_id){
    $object_array = array();
    $get_post_data =  DB::table('product')
                      ->where(['product.post_status' => 1, 'product.post_type' => 'product', 'product_relationships.term_id' => $term_id ])
                      ->join('product_relationships', 'product_relationships.object_id', '=', 'product.id')
                      ->select('product.*')
                      ->orderBy('product.id', 'desc')
                      ->get()
    ...
    if(count($related_items) > 0){
      ...
      if(count($products_id_array) > 0){
        foreach($products_id_array as $related_products_id){
          $get_post_meta  =  ProductExtra:: where(['post_id' => $related_products_id, 'key_name' => '_product_enable_as_related'])->first();
  ...
  public function manageExportProducts(){
    $export_data = array();
    $get_products = Product:: where(['post_type' => 'product'])->orderBy('id', 'DESC')->get()->toArray();
    ...
    if(count($get_products) > 0){
      foreach($get_products as $rows){
        ...
        $get_post_extras = ProductExtra::where(['post_id' => $rows['id']])->get()->toArray();
        if(count($get_post_extras) > 0){
          foreach($get_post_extras as $post_extras){
            ...
            if($post_extras['key_name'] == '_product_sale_price'){
              $product_sale_price = $post_extras['key_value'];
            }
        ...
        array_push($export_data, array('title' => $rows['post_title'], 'description' => $rows['post_content'], 'regular_price' => $regular_price, 'sale_price' => $product_sale_price, 'sku' => $sku, 'extra_features' => $short_features, 'enable_recommended' => ($recommended) ? 'TRUE' : 'FALSE', 'enable_features' => ($features) ? 'TRUE' : 'FALSE', 'enable_latest' => ($latest) ? 'TRUE' : 'FALSE', 'enable_related' => ($related) ? 'TRUE' : 'FALSE', 'enable_home' => ($home_page) ? 'TRUE' : 'FALSE', 'visibility' => ($visibility) ? 'TRUE' : 'FALSE', 'reviews' => ($reviews) ? 'TRUE' : 'FALSE', 'p_reviews' => ($p_reviews) ? 'TRUE' : 'FALSE', 'd_reviews' => ($d_reviews) ? 'TRUE' : 'FALSE', 'seo_title' => $seo_title, 'seo_desc' => $seo_desc, 'seo_keywords' => $seo_keywords));
      }
    }
    ...
    $columns = array('Title', 'Description(HTML)', 'Regular Price', 'Sale Price', 'SKU', 'Features(HTML)', 'Recommended Product', 'Features Product', 'Latest Product', 'Related Product', 'Home Page Product', 'Visibility', 'Enable Reviews', 'Enable Review Product Page', 'Enable Review Details Page', 'SEO Title', 'SEO Description', 'SEO Keywords(Comma Separator)');
    ...
    $callback = function() use ($export_data, $columns){
      ...
      if(count($export_data) > 0){
        foreach($export_data as $data) {
          fputcsv($file, array($data['title'], $data['description'], $data['regular_price'], $data['sale_price'], $data['sku'], $data['extra_features'], $data['enable_recommended'], $data['enable_features'], $data['enable_latest'], $data['enable_related'], $data['enable_home'], $data['visibility'], $data['reviews'], $data['p_reviews'], $data['d_reviews'], $data['seo_title'], $data['seo_desc'], $data['seo_keywords']));
        }
      }
