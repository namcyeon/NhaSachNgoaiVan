...
class AdminDashboardController extends Controller
{
    ...
    if( isset($getRoutePrefix) && $getRoutePrefix == 'admin' && !Session::has('shopist_admin_user_id') ){
      ...
      if(Request::is('admin/product/categories/list') || Request::is('admin/blog/categories/list')){
        ...
        if(Request::is('admin/blog/categories/list')){
          $this->dashboardData['cat_list_data']        =   $this->post->getTermData( 'blog_cat', true, $search_value, -1 );
          $this->dashboardData['only_cat_name']        =   $this->post->get_categories_name('blog_cat');
          $this->dashboardData['search_value']         =   $search_value;
      ...
      if(Request::is('admin/manufacturers/list')){
        ...
        $this->dashboardData['manufacturerslist']   =   $this->post->getTermData( 'product_brands', true, $search_value, -1 );
        $this->dashboardData['search_value']        =   $search_value;
      }
      ...
      if(Request::is('admin/designer/clipart/categories/list')){
        ...
        $this->dashboardData['art_cat_lists_data']   =   $this->post->getTermData( 'designer_cat', true, $search_value, -1 );
        $this->dashboardData['search_value']         =   $search_value;
      }
      ...
      if(Request::is('admin/designer/clipart/add') ||  Request::is('admin/designer/clipart/update/*')){
        $this->dashboardData['getArtCatByFilter']   =   $this->post->getTermData( 'designer_cat', false, null, 1 );
      }
      ...
      if(Request::is('admin/blog/add') || ($params && Request::is('admin/blog/update/*'))){
        $this->dashboardData['blog_categories_lists']     =   $this->post->get_categories( 0, 'blog_cat');
      }
      ...
      if($params && Request::is('admin/manufacturers/update/*')){
        $getBrandsData = ProductTerm:: where('slug', $params)->first();
     ...
     if($params && Request::is('admin/product/update/*')){
      ...
      if(count($get_post) > 0){
        ...
        $get_post_attr  =  ProductExtra::where(['post_id' => $product_id, 'key_name' => '_attribute_post_data'])->first();
  ...
  if(Request::is('admin/orders') || Request::is('admin/orders/current-date')){
    ...
    if(count($get_shop_order_data) > 0){
      foreach($get_shop_order_data as $row){
        $get_postmeta_by_order_id = ProductExtra::where(['post_id' => $row['id']])->get();
  ...
  if(Request::is('admin/orders/details/*') && $params){
    $get_post_by_order_id     = Product::where(['id' => $params])->first();
    $get_postmeta_by_order_id = ProductExtra::where(['post_id' => $params])->get();
 ...
 if(Request::is('admin/dashboard')){
  ...
  $totalProducts      =   Product::where(['post_type' => 'product', 'post_status' => 1])->get();
  $todayOrders        =   Product::whereDate('created_at', '=', $this->carbonObject->today()->toDateString())->where('post_type', 'shop_order')->get();
  $totalOrders        =   Product::where('post_type', 'shop_order')->get();
  $last2DaysOrders    =   Product::whereBetween('created_at', array($this->carbonObject->yesterday()->toDateString(), $this->carbonObject->today()->toDateString().' 23:59:59'))->where('post_type', 'shop_order')->orderBy('created_at', 'DESC')->get();
  $lastProducts       =   Product::whereBetween('created_at', array($this->carbonObject->yesterday()->toDateString(), $this->carbonObject->today()->toDateString().' 23:59:59'))->where(['post_type' => 'product', 'post_status' => 1])->orderBy('created_at', 'DESC')->get();
 ...
 if($todayOrders && $todayOrders->count() > 0){
  ...
  foreach($todayOrders as $rows){
    $get_order_total = ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_order_total'])->first();
...
if($last2DaysOrders && $last2DaysOrders->count() > 0){
  foreach($last2DaysOrders as $rows){
    ...
    $order_status                 =   ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_order_status'])->first();
    if(!empty($order_status->key_value)){
      $order_data['order_status'] =   $order_status->key_value;
    }

    $order_total                  =   ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_order_total'])->first();
    if(!empty($order_total->key_value)){
      $order_data['order_totals'] =   $order_total->key_value;
    }

    $order_currency               =   ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_order_currency'])->first();
...
if($lastProducts && $lastProducts->count() > 0){
  foreach($lastProducts as $rows){
    $products_data = array();
    $parse_url = json_decode(ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_product_related_images_url'])->first()->key_value);    
    ...
    if(get_product_type($rows->id) == 'simple_product'){
      if(!empty(ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_product_price'])->first()->key_value)){
        $products_data['price'] = ProductExtra::where(['post_id' => $rows->id, 'key_name' => '_product_price'])->first()->key_value.' '.get_current_currency_symbol();
      }
      else
      {
        $products_data['price'] = '0 '.get_current_currency_symbol();
      }
    }
    ...
    if( Request::isMethod('post') && Session::token() == Input::get('_token'))
      {
        ...
        if( ProductExtra::where(['post_id' => $order_id, 'key_name' => '_order_status'])->update( $data ))
          {
            return redirect()->back();
          }
  ...
  public function sendQuickMail()
  {
    if( Request::isMethod('post') && Session::token() == Input::get('_token') )
      ...
      else
      {
        ...
        $this->classGetFunction->sendSimpleMail( $mailData );
        
        Session::flash('success-message', 'Đã gửi thành công');
        return redirect()->back();
      }
    }
    else 
